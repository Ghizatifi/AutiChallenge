...
import ma.auti.afor.autichallenge.Model.Score;
...
public class Db2 extends SQLiteOpenHelper {
    ...
    //CRUD For Table
    public List<QuestionNombre> getAllQuestion() {
        List<QuestionNombre> listQuestion = new ArrayList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c;
        try {
            c = db.rawQuery("SELECT * FROM QuestionNombre ORDER BY Random()", null);
            ...
            do {
                ...
                QuestionNombre question = new QuestionNombre(Id, Image, AnswerA, AnswerB, AnswerC, CorrectAnswer);
    ...
    public List<QuestionNombre> getQuestionMode(String mode) {
        List<QuestionNombre> listQuestion = new ArrayList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c;
        int limit = 0;
        if (mode.equals(Common.MODE.EASY.toString()))
            ...
            limit = 200;
        try {
            c = db.rawQuery(String.format("SELECT * FROM QuestionNombre ORDER BY Random() LIMIT %d", limit), null);
            if (c == null) return null;
            c.moveToFirst();
            do {
                int Id = c.getInt(c.getColumnIndex("ID"));
