package ma.auti.afor.autichallenge.DB;
...
import ma.auti.afor.autichallenge.Common.*;
import ma.auti.afor.autichallenge.Model.*;
...
public class Db1 extends SQLiteOpenHelper {
    ...
    private static String DB_NAME = "MyDB2.db";
    ...
    public Db1(Context context) {
        ...
        File file = new File(DB_PATH+"MyDB2.db");
        if(file.exists())
            openDataBase(); // to fix db.insert can't insert values
    ...
    //CRUD For Table
    public List<QuestionFruit> getAllQuestion() {
        List<QuestionFruit> listQuestion = new ArrayList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c;
        try {
            c = db.rawQuery("SELECT * FROM QuestionLFruit ORDER BY Random()", null);
            ...
            do {
                ...
                QuestionFruit question = new QuestionFruit(Id, Image, AnswerA, AnswerB, AnswerC, CorrectAnswer);
    ...
    public List<QuestionFruit> getQuestionMode(String mode) {
        List<QuestionFruit> listQuestion = new ArrayList<>();
        SQLiteDatabase db = this.getWritableDatabase();
        Cursor c;
        int limit = 0;
        if (mode.equals(Common.MODE.EASY.toString()))
            ...
            limit = 200;
        try {
            c = db.rawQuery(String.format("SELECT * FROM QuestionFruit ORDER BY Random() LIMIT %d", limit), null);
            if (c == null) return null;
            c.moveToFirst();
            do {
                int Id = c.getInt(c.getColumnIndex("ID"));
    ...
    public List<Score> getRanking() {
        List<Score> listRanking = new ArrayList<>();
        ...
        try {
            ...
            do {
                ...
                Score score = new Score(Id, Score);
                listRanking.add(score);
            } while (c.moveToNext());
            c.close();
        } catch (Exception e) {
            e.printStackTrace();
